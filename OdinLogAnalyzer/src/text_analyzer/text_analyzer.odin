...
init_state :: proc() -> State {
    ...
    font_path := "src/resources/CascadiaCode/CaskaydiaCoveNerdFont-Regular.ttf"
    font_cstr := strings.clone_to_cstring(font_path)
    defer delete(font_cstr)
    font := rl.LoadFont(font_cstr)
...
render_menu :: proc(state: ^State) {
    ...
    if rl.GuiButton({x, y, button_width, button_height}, "Open") {
        ...
        if os.exists(log_file) {
            load_file(state, log_file)
        } else if os.exists("example.txt") {
            load_file(state, "example.txt")
        }
    }
...
render_find_dialog :: proc(state: ^State) {
    ...
    // Find button
    find_rect := rl.Rectangle{dialog_x + 10, button_y, button_width, button_height}
    if rl.GuiButton(find_rect, "Find") {
    ...
    // Show find history
    history_y := dialog_y + 100
    for i := 0; i < len(state.find_history); i += 1 {
        history_text := state.find_history[i]
        history_cstr := strings.clone_to_cstring(history_text)
        defer delete(history_cstr)
        rl.DrawTextEx(state.font, history_cstr, {dialog_x + 10, history_y + f32(i * 20)}, 14, 1, colors.dialog_text)
    }
}
